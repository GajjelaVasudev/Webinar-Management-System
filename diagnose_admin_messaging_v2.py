"""
Diagnostic script v2 - Using proper DRF token authentication
"""
...
def diagnose_admin_messaging_v2():
    """Diagnose the admin messaging issue with proper authentication"""
    print("=" * 70)
    print("🔍 DIAGNOSING ADMIN MESSAGING ISSUE (v2 - Proper Auth)")
    print("=" * 70)
    ...
    student_user, _ = User.objects.get_or_create(
        ...
        defaults={'email': 'student_v2@test.com', 'first_name': 'Test', 'last_name': 'Student'}
    )
    admin_user, _ = User.objects.get_or_create(
        username='test_admin_v2',
        defaults={'email': 'admin_v2@test.com', 'first_name': 'Test', 'last_name': 'Admin', 'is_staff': True, 'is_superuser': True}
    )
    ...
    try:
        ...
        force_authenticate(request, user=admin_user)  # Proper authentication
        ...
        if response.status_code == 200:
            ...
            print(f"   ✓ Number of conversations returned: {len(response.data)}")
            ...
            if len(response.data) > 0:
                ...
                print(f"   ❌ ERROR: No conversations returned for admin!")
    ...
    print("=" * 70)
    print("\n✅ Results:")
    print("   ✓ Admin can fetch conversations: Backend is working properly")
    print("   ✓ Admin can fetch messages: Permissions are correct")
    print("   ✓ Admin can send messages: Reply functionality works")
    print("\n💡 If backend works, the issue is likely in the FRONTEND.")
    print("   - Check browser console for JavaScript errors")
    print("   - Check if InboxPage.tsx is loading correctly")
    print("   - Verify API calls are being made with correct tokens")
    print("\n")


if __name__ == '__main__':
    diagnose_admin_messaging_v2()
